...
export const Product: React.FC = () => {
  ...
  const handleNetworkChange = (event)=>{
    ...
    try {
      ...
      let _ipfs_cid = ipfsCid;
      const urlParams = new URLSearchParams(window.location.search);
      console.log('params______________________');
      console.log(urlParams.get('ipfs_cid'));
      if (urlParams.get("ipfs_cid")) {
          _ipfs_cid = urlParams.get("ipfs_cid");
      }
      alert("ipfs_cid " + _ipfs_cid);
      const claim_request_response = await claim_request(api_details_ref.current.api_base_url, session_ref.current, _ipfs_cid);
